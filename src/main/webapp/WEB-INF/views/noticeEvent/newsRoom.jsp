...
<html>
	...
	<body>
        ...
        <main>
            ...
            <div id="main_middle">
                <h2><b>1인가구 실시간 뉴스</b></h2>
                <hr>
                <div class="search">
                    <form action="/newsRoom/search.do" method="get">
						<input type="text" name="searchKeyword" placeholder="다른 뉴스 검색" style="width:30%">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody class="table-group-divider">
                    	<c:if test="${pInfo.currentPage == 1 }">
							<c:forEach var="newsRoom" items="${nList}" begin="0" end="9">
								<tr>
						...
						<c:if test="${pInfo.currentPage == 2 }">
							<c:forEach var="newsRoom" items="${nList}" begin="10" end="19">
								<tr>
									<td><a href="${newsRoom.newsLink }" target="_blank">${newsRoom.newsTitle }</a></td>
									<td style="text-align:center">${newsRoom.newsSource }</td>
									<td style="text-align:center">${newsRoom.createDate }</td>
						...
						<c:if test="${pInfo.currentPage == 3 }">
							<c:forEach var="newsRoom" items="${nList}" begin="20" end="29">
								<tr>
									<td><a href="${newsRoom.newsLink }" target="_blank">${newsRoom.newsTitle }</a></td>
									<td style="text-align:center">${newsRoom.newsSource }</td>
									<td style="text-align:center">${newsRoom.createDate }</td>
						...
						<c:if test="${pInfo.currentPage == 4 }">
							<c:forEach var="newsRoom" items="${nList}" begin="30" end="39">
								<tr>
									<td><a href="${newsRoom.newsLink }" target="_blank">${newsRoom.newsTitle }</a></td>
									<td style="text-align:center">${newsRoom.newsSource }</td>
									<td style="text-align:center">${newsRoom.createDate }</td>
						...
						<c:if test="${pInfo.currentPage == 5 }">
							<c:forEach var="newsRoom" items="${nList}" begin="40" end="49">
								<tr>
									<td><a href="${newsRoom.newsLink }" target="_blank">${newsRoom.newsTitle }</a></td>
									<td style="text-align:center">${newsRoom.newsSource }</td>
									<td style="text-align:center">${newsRoom.createDate }</td>
					...
					<tfoot>
						<tr align="center">
							<td colspan="3">
							<c:if test="${pInfo.currentPage != 1 }">
								<a href="/newsRoom/list.do?currentPage=${pInfo.currentPage - 1 }">&lt;</a>&nbsp;
							</c:if>
							<c:forEach begin="${pInfo.startNavi }" end="${pInfo.endNavi }" var="p">
								<c:url var="pageUrl" value="/newsRoom/list.do">
